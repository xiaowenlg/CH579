...
static uint8 simpleProfileChar2[SIMPLEPROFILE_CHAR2_LEN] = { 'm' };
...
static gattAttribute_t simpleProfileAttrTbl[] = 
{
  ...
  { 
    ...
    // Characteristic 1 Declaration            // 1，特征声明
    { 
      { ATT_BT_UUID_SIZE, characterUUID },     //特征类型
      GATT_PERMIT_READ, 												//特征权限
      0,																				//特征句柄
      &simpleProfileChar1Props 									//特征值        此处声明特征权限
    },

      // Characteristic Value 1									//2，特征值
      { 
        { ATT_BT_UUID_SIZE, simpleProfilechar1UUID },  //uuid绑定到特征
        GATT_PERMIT_READ | GATT_PERMIT_WRITE, 			//特征权限
        0, 
        simpleProfileChar1                         //特征值
      },

      // Characteristic 1 User Description				
      { 
        { ATT_BT_UUID_SIZE, charUserDescUUID },//3 特征描述
        GATT_PERMIT_READ, 
        0, 
...
bStatus_t SimpleProfile_SetParameter( uint8 param, uint8 len, void *value )
{
  ...
  switch ( param )
  {
    case SIMPLEPROFILE_CHAR1:
      if ( len == SIMPLEPROFILE_CHAR1_LEN )    //检测数据完整性
      {
...
{
  uint16 value = GATTServApp_ReadCharCfg( connHandle, simpleProfileChar4Config );//返回特性值
                                 ...
                                 uint8 *pValue, uint16 len, uint16 offset,uint8 method  )
{
  ...
  // If a charactersitic value changed then callback function to notify application of change  //如果特征值改变，然后调用函数处理应用的变化
